...
lipids_pgs.long.assoc <- assoc_sig_results_pheno %>% inner_join(lipids_pgs.long, ., by = "lipid")
...
library(broom)
tmp_all_asd_stats <- lipids_pgs.long.assoc %>% 
  nest(data = -lipid) %>% 
...
lipids_pgs_r2_agesex_asd <- inner_join(tmp_all_asd_stats, tmp, by = "lipid")
...
tmp_neurodev_asd_stats <- lipids_pgs.long.assoc %>% 
  filter(Pheno == "ASD") %>% 
  nest(data = -lipid) %>% 
  mutate(model = purrr::map(data, ~aov(pmol_mL ~ lipid_PGS_EUR + age + age2 + sex + ASD, data = .)), tidied = purrr::map(model, tidy)) %>%
  unnest(tidied) %>% 
...
tmp_neurodev_iqdq_stats <- lipids_pgs.long.assoc %>% 
  filter(Pheno == "IQ/DQ") %>% 
  nest(data = -lipid) %>% 
...
tmp_neurodev_sleep_stats <- lipids_pgs.long.assoc %>% 
...
lipids_pgs_r2_agesex_batch_asd <- inner_join(tmp_all_asd_stats, tmp, by = "lipid")
...
lipids_pgs_r2_agesex_batchtime_asd <- inner_join(tmp_all_asd_stats, tmp, by = "lipid")
...
lipids_pgs_r2_agesex_batch_diet_asd <- inner_join(tmp_all_asd_stats, tmp, by = "lipid")
